...
export default function DishSection() {
  ...
  return (
    // Semi-transparent — mesh glows through the warm beige
    <section style={{ background:"rgba(237,224,204,0.70)", minHeight:"100vh", display:"flex", alignItems:"center", backdropFilter:"blur(2px)" }} className="py-24">
      <div className="max-w-7xl mx-auto px-6 w-full">
        ...
        <div ref={headRef} style={{ textAlign:"center", maxWidth:700, margin:"0 auto 64px", opacity:headIn?1:0, transform:headIn?"translateY(0)":"translateY(30px)", transition:"all 0.7s ease" }}>
          <p style={{ color:t.chocolateMid, fontFamily:t.fontBody, fontSize:11, letterSpacing:"0.3em", textTransform:"uppercase", marginBottom:20 }}>Dish Ratings</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="grid grid-cols-2 gap-4">
            {DISHES.map((dish, i) => (
              <button key={dish.name} onClick={() => setActive(i)}
                style={{ background: active===i ? t.chocolateDeep : "rgba(253,251,247,0.85)", backdropFilter:"blur(10px)", WebkitBackdropFilter:"blur(10px)", borderRadius:"1.25rem", padding:"22px 20px", cursor:"pointer", transition:"all 0.3s", border: active===i ? "none" : `1px solid ${t.beigeDark}50`, boxShadow: active===i ? `0 12px 32px ${t.chocolateDeep}40` : `0 2px 12px ${t.chocolateDeep}0A`, textAlign:"left", width:"100%" }}>
                <p style={{ color: active===i ? t.cream : t.chocolateDeep, fontFamily:t.fontHeading, fontSize:18, fontWeight:600, marginBottom:8, lineHeight:1.3 }}>{dish.name}</p>
          ...
          <div style={{ background:t.chocolateDeep, borderRadius:"1.75rem", padding:"40px 36px", position:"relative", overflow:"hidden" }}>
            ...
            <div style={{ marginBottom:10, position:"relative", zIndex:2 }}>
              <p style={{ color:t.chocolateLight, fontFamily:t.fontBody, fontSize:11, letterSpacing:"0.25em", textTransform:"uppercase", marginBottom:10 }}>Selected Dish</p>
              <h3 style={{ color:t.cream, fontFamily:t.fontHeading, fontSize:32, fontWeight:600, marginBottom:6 }}>{DISHES[active].name}</h3>
              <p style={{ color:t.beigeDark, fontFamily:t.fontHeading, fontSize:24 }}>₹{DISHES[active].price}</p>
            </div>
            <div style={{ width:"100%", height:1, background:`${t.chocolateLight}30`, margin:"24px 0", position:"relative", zIndex:2 }} />
            <p style={{ color:t.chocolateLight, fontFamily:t.fontBody, fontSize:14, lineHeight:1.8, marginBottom:28, position:"relative", zIndex:2, maxWidth:320 }}>{DISHES[active].desc}</p>
